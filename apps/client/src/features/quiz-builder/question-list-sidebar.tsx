...
function Row({ question, index, selected, onSelect, onRemove }: { question: Question; index: number; selected: boolean; onSelect: () => void; onRemove: () => void }) {
...
export function QuestionListSidebar({ questions, selectedId, onSelect, onReorder, onRemove }: Props) {
    ...
    <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={questions.map((q) => q.id)} strategy={verticalListSortingStrategy}>
        <div className="space-y-1">
          {questions.map((q, i) => (
            <Row key={q.id} question={q} index={i} selected={q.id === selectedId} onSelect={() => onSelect(q.id)} onRemove={() => onRemove(q.id)} />
          ))}
        </div>
      </SortableContext>
    </DndContext>
